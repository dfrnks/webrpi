...
def getIpAddress():
    f = os.popen('ifconfig eth0 | grep "inet\ end" | cut -d: -f2 | cut -d" " -f2')
    return f.read()
...
def get_ip():
    ...
    for iface in [' '.join(i) for i in iter(lambda: list(itertools.takewhile(lambda l: not l.isspace(),f)), [])]:
        if re.findall('^(eth|wlan|lxcbr)[0-9]',iface) and re.findall('RUNNING',iface):
            ip = re.findall('(?<=inet\send.: )[0-9\.]+',iface)
            if ip:
                ips.append(ip[0])
    return ips
